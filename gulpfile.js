...
const PATH = {
  ...
  pugAllFiles: './src/pug/**/*.pug',
  pugFolder: './templates/',
  pugRootFile: './src/pug/index.pug',
}
...
function compileScssMinify() {
  const plaginsForMinify = [...PLUGINS, cssnano({preset: 'default'})]
  return src(PATH.scssRootFile)
    .pipe(sass().on('error', sass.logError))
    .pipe(postcss(plaginsForMinify))
...
function watchFiles() {
  syncInit();
  watch(PATH.scssAllFiles, series(compileScss, compileScssMinify))
...
task('min', compileScssMinify)
task('scss', series(compileScss, compileScssMinify))
task('comb', comb)
